...
class RFPW_Widget extends WP_Widget {
	...
	*/
	...
	public function form( $instance ) {
		...
		$featured_image = isset( $instance['featured_image'] ) ? $instance['featured_image'] : '';
...
?>
		...
		<p>
		<input id="<?php echo $this->get_field_id( 'featured_image' ); ?>" class="checkbox" type="checkbox" <?php checked( $featured_image, 'on' ); ?> name="<?php echo $this->get_field_name( 'featured_image' ); ?>"></input>
		<label for="<?php echo $this->get_field_id( 'featured_image' ); ?>">
		<?php _e( 'Display featured image (if any)?', 'rfpw_widget' ); ?></label>
		</p>
...
<?php 
	...
	public function widget( $args, $instance ) {	
		...
		while ( $query->have_posts() ) {
			...
			/* If user chooses to display featured image and it exists, get the image */
			if ( $featured_image == 'on' && has_post_thumbnail() ) {
				$feat_img = wp_get_attachment_image_src( get_post_thumbnail_id(), 'thumbnail' );
				$img_arr[0] = $feat_img[0];
			}
			else {
				$img = get_attached_media( 'image' ); // Get all images attached to the post
			
				// Get the oldest image attached to the post, then break the loop
			...
			if ( $show_excerpt == 'on' ) {
				...
				$show_content .= 
					'<li class="rfpw-show-content" style="background-image: url(' .$img_arr[0] . ')">'
					 . $image . 
